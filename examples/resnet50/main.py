...
parser.add_argument('--local_rank', metavar='local_rank',
                    help='local rank')
...
parser.add_argument('--gpu', default=None, type=int,
                    help='GPU id to use.')
parser.add_argument('--sycl', default=None, type=int,
                    help='SYCL id to use.')
parser.add_argument('--fp16', default=0, type=int, help='Datatype used: fp32/fp16')
...
class DummyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dir, lable_size, dataset_size, transform):
        """Init DummyDataset."""
        super(DummyDataset, self).__init__()
        self.images = [d.path for d in os.scandir(dir)]
    ...
    def __getitem__(self, index):
        ...
        image = pil_loader(self.images[0])
        image = self.transform(image)
        return image, random.randint(0, self.lable_size)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained_weights:
        if os.path.isfile(args.pretrained_weights):
            ...
            print("=> no pretrained weights found at '{}'".format(args.pretrained_weights))
    ...
    #         normalize,
    #     ]))
    train_dataset = DummyDataset(traindir, 300, 30000, transforms.Compose([
            transforms.RandomResizedCrop(224),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            normalize,
    ...
    val_loader = torch.utils.data.DataLoader(
        DummyDataset(valdir, 300, 1000, transforms.Compose([
            transforms.Resize(256),
            transforms.CenterCrop(224),
            transforms.ToTensor(),
            normalize,
