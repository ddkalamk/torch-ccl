...
template <>
Comms& CCLCommsCollector<CPU>::get_ccl_comms(const std::string& devices_key, const std::vector<at::Device>& devices) {
  // Sanity check
  ...
  std::shared_ptr<Comms> cpu_comms_ptr = get_ccl_comms_(devices_key);
  if (cpu_comms_ptr) {
  ...
  cpu_comms_ptr = std::make_shared<Comms>(cpu_comms);
